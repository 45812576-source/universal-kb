...
```
- 每次回复最多输出一个结构化块（studio_summary / studio_draft / studio_diff / studio_test_result 四选一）。JSON 必须合法。
- 不要解释这些代码块的格式，直接输出。"""
...
_BLOCK_PATTERN = re.compile(
    r"```(studio_draft|studio_diff|studio_test_result|studio_summary)\s*\n([\s\S]*?)\n```",
    re.IGNORECASE,
)
