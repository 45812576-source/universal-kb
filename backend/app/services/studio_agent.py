...
- 除 studio_tool_suggestion 外，其他结构化块（studio_summary / studio_draft / studio_diff / studio_test_result）每次回复最多输出一个。JSON 必须合法。
- 不要解释这些代码块的格式，直接输出。"""
...
_BLOCK_PATTERN = re.compile(
    r"```(studio_draft|studio_diff|studio_test_result|studio_summary|studio_tool_suggestion)\s*\n([\s\S]*?)\n```",
    re.IGNORECASE,
)
...
async def run_stream(
    ...
    system_content = _build_system(selected_skill_id, editor_prompt, editor_is_dirty, available_tools)
