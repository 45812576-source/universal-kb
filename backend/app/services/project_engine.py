...
_TODO_REMINDER_PROMPT = """你是项目跟进助手。根据以下项目近期进展，生成一条早间跟进提醒，不超过80字，鼓励团队跟进待办事项。

项目名称: {project_name}
近期进展摘要:
{summary_text}

将提醒消息放在 <reminder> 标签内输出，例如：
<reminder>早安团队！昨日确定了方案，今天请重点跟进接口缓存改造，加油！</reminder>"""
...
class ProjectEngine:
    ...
    async def inject_todo_reminder(self, project, db: Session) -> None:
        ...
        # 从 <reminder>...</reminder> 标签提取
        _tag_match = re.search(r'<reminder>(.*?)</reminder>', reminder_text, re.DOTALL)
        if _tag_match:
            reminder_text = _tag_match.group(1).strip()
