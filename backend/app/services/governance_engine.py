...
def auto_apply_governance(db: Session, entry: KnowledgeEntry, result: dict[str, Any]) -> None:
    ...
    entry.governance_confidence = result["confidence"] / 100.0
    entry.governance_note = f"自动生效：{result['reason']}"

    if result.get("objective"):
        entry.governance_objective_id = result["objective"].id
    if result.get("library"):
        entry.resource_library_id = result["library"].id
    if result.get("object_type"):
        entry.object_type_id = result["object_type"].id
    if result.get("kr"):
        entry.governance_kr_id = result["kr"].id
    if result.get("element"):
        entry.governance_element_id = result["element"].id
    ...
    if strategy_key:
        record_governance_feedback(
            ...
            reward=0.8,
            created_by=None,
            to_objective_id=entry.governance_objective_id,
            to_resource_library_id=entry.resource_library_id,
            note="自动生效",
        )
    ...
    task = GovernanceSuggestionTask(
        ...
        status="applied",
        objective_id=entry.governance_objective_id,
        resource_library_id=entry.resource_library_id,
...
def create_review_suggestion(db: Session, entry: KnowledgeEntry, result: dict[str, Any]) -> GovernanceSuggestionTask:
    ...
    entry.governance_confidence = result["confidence"] / 100.0
    entry.governance_note = result["reason"]
...
def _collect_stats_data(db: Session) -> dict:
    ...
    coverage_rate = round(aligned / max(total_entries, 1) * 100, 1)
