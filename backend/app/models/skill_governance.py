...
class TestCasePlanDraft(Base):
    ...
    source_plan_id = Column(Integer, ForeignKey("test_case_plan_drafts.id", ondelete="SET NULL"), nullable=True)
    generation_mode = Column(String(32), nullable=True)
    entry_source = Column(String(32), nullable=True)
    conversation_id = Column(Integer, nullable=True)
    summary_json = Column(JSON, nullable=True)
    confirmed_at = Column(DateTime, nullable=True)
    latest_materialized_session_id = Column(Integer, nullable=True)
    last_used_at = Column(DateTime, nullable=True)
    created_by = Column(Integer, ForeignKey("users.id"), nullable=False)
    created_at = Column(DateTime, default=datetime.datetime.utcnow)

    skill = relationship("Skill", foreign_keys=[skill_id])
    source_plan = relationship("TestCasePlanDraft", foreign_keys=[source_plan_id], remote_side="TestCasePlanDraft.id")
    bundle = relationship("RolePolicyBundle", foreign_keys=[bundle_id])
